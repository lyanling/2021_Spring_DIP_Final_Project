...
def adjust_connect_list(connect_list, label, shift_value):
	...
	for connect_label in connect_labels:
		connect_list[connect_label-1][label][0] += shift_value[0]
		connect_list[connect_label-1][label][1] += shift_value[1]

def shift_part(parts, label, shift_value):
	for point in parts[label-1]:
		point[0] += shift_value[0]
		point[1] += shift_value[1]
